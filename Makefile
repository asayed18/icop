...
help:
	...
	@echo   make install_plugin Build the release and install it into Windows VLC
...
install_plugin: release
	"$(POWERSHELL)" -NoProfile -ExecutionPolicy Bypass -File "tools/install_icop_plugin.ps1" -ReleaseRoot "$(RELEASE_ROOT)" $(VERSION_INSTALL_ARG) $(VLC_ROOT_INSTALL_ARG) $(INSTALL_ARGS)
